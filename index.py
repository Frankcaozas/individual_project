...
def plot(schedle1, schedule2):
    ...
    def p(machine_schedules):
        ...
        gnt.set_yticklabels(['Machine {}'.format(i+1) for i in machine_ids])
...
priority, part_code, processing_quantity, part_specification,  material = data_array
...
for i in range(num_machines):
    print(f"Machine {i}:")
    s = machine_shcedules[i]
    m = [material[i] for i in s]
    p = [priority[i] for i in s]
    q = [processing_quantity[i] for i in s]
    test(m, p, q)
